...

{
    ...
    },
    {
        ...
        options: ["Buenos Aires", "Rio de Janeiro", "BrasÃ­lia", "Sao Paulo"],
        correctAnswer: 2,
    },
    {
...

{
    ...
    {
        question: "What is 15 Ã— 13?",
        options: ["180", "185", "195", "200"],
        correctAnswer: 2,
    },
    {
        question: "What is the value of 8Â³?",
    ...
    {
        question: "What is the value of 3 + 5 Ã— 4?",
        options: ["20", "22", "24", "23"],
        correctAnswer: 3,
    },
    {
    ...
    {
        question: "What is 11Â²?",
        options: ["121", "131", "141", "111"],
        correctAnswer: 3,
    },
    {
        question: "What is 9 Ã— 12?",
        options: ["105", "110", "108", "120"],
        correctAnswer: 2,
    },
    {
        question: "What is the value of 16 Ã· 4?",
        options: ["2", "3", "4", "5"],
        correctAnswer: 2,
    },
    {
    ...
    },
    {
        ...
        options: ["40 cmÂ²", "50 cmÂ²", "55 cmÂ²", "60 cmÂ²"],
        correctAnswer: 0,
    },
    {
        question: "What is the value of 10 Ã· 2 + 3?",
        options: ["8", "7", "9", "6"],
        correctAnswer: 1,
    },
    {
        question: "What is 3 Ã— 7 + 2?",
        options: ["20", "21", "22", "23"],
        correctAnswer: 1,
    },
    {
    ...
    {
        question: "What is the value of 2Â³ Ã— 3?",
        options: ["12", "15", "18", "24"],
        correctAnswer: 0,
    },
    {
        question: "What is the value of 10 Ã— (5 + 3)?",
        options: ["80", "70", "60", "50"],
        correctAnswer: 1,
    },
    {
        question: "What is the value of 14 Ã— 14?",
    ...
    {
        question: "What is the area of a circle with a radius of 3 cm? (Use Ï€ = 3.14)",
        options: ["28.26 cmÂ²", "31.42 cmÂ²", "36.14 cmÂ²", "39.14 cmÂ²"],
        correctAnswer: 0,
    },
    {
        question: "What is the value of (8 + 2) Ã— 3?",
        options: ["30", "32", "34", "28"],
        correctAnswer: 3,
    },
    {
    ...
    {
        question: "What is the value of 25 Ã· 5 Ã— 3?",
        options: ["12", "15", "18", "20"],
        correctAnswer: 1,
    },
    ],
...
const quizConfig = document.querySelector(".quiz_configuration_container");
...
const resultMsg = resultContainer.querySelector("p");
const tryAgainBtn = resultContainer.querySelector("button");
const categoryOptions = document.querySelector(".category_options");
const userSelectQuestion = document.querySelector(".question_numbers");

let selectedQuiz = [];
let selectedCorrect = [];
let questionIndex = 0;

const loadQuiz = (quizSet) => {
    const { question, options } = quizSet[questionIndex];
    quizQuestion.innerHTML = `${questionIndex + 1}. ${question}`;
    quizOptions.innerHTML = "";
    options.forEach((opt) => {
        const optionElement = document.createElement("label");
        optionElement.innerHTML = `
            <input type="radio" name="q${questionIndex + 1}" value="${opt}">
            <span>${opt}</span>
        `;
        quizOptions.appendChild(optionElement);
    })
    questionNumber.innerHTML = `${questionIndex + 1} of ${selectedQuiz.length} Questions`;
}

const submitConfiguration = (event) => {
    event.preventDefault();

    const configData = new FormData(quizConfig);
    const { category, question } = Object.fromEntries(configData);

    if (!category && !question) {
        alert("Plz Select category and question");
        return;
    } else if (!category) {
    ...
    const filteredQuizSet = questionBank.filter((questionSet) => questionSet.category  === category.toLowerCase())[0].questions;
    selectedQuiz = filteredQuizSet.slice(0, question);

    for (let val of selectedQuiz) {
        const obj = {
            options: val.options,
            correctAnswer: val.correctAnswer
        }
        selectedCorrect.push(obj)
    }
...
const answersData = new FormData(quizContainer);
let userAnswerData = Object.fromEntries(answersData);

const updateNextQuiz = (event) => {
    event.preventDefault();
...
const checkAnswer = (event) => {
    if (event.target.tagName === "INPUT") {
        ...
        userAnswerData[input.name] = input.value;
        
        const { options, correctAnswer } = selectedQuiz[questionIndex];

        if ( options[correctAnswer] === input.value) {
            input.parentElement.classList.add("correct");
        } else {
            input.parentElement.classList.add("wrong");
            quizOptions.querySelectorAll("label").forEach((opt) => {
                if (opt.children[0].value === options[correctAnswer]) {
                    opt.children[1].parentElement.classList.add("correct");
                }
            })
        }

        quizOptions.removeEventListener("click", checkAnswer);
    }
}
...
const onSubmit = (event) => {
    ...
    selectedCorrect.forEach((obj, index) => {
        correct[`q${index + 1}`] = obj.options[obj.correctAnswer];
    })

    for (let [key, val] of Object.entries(userAnswerData)) {
        if (correct[key] === val) {            
            count++
        }
    }

    resultMsg.innerHTML = `You answered ${count} out of ${selectedQuiz.length} questions correctly. Great effort!`;
...
const replayQuiz = () => {
    ...
    document.querySelectorAll("input:checked").forEach((inp) => {
        inp.parentElement.classList.remove("check");
        inp.checked = false;
    })
...
const handleConfig = (event) => {
    if (event.target.tagName === "INPUT") {
        ...
        categoryOptions.querySelectorAll("input").forEach((inp) => {
            if (input !== inp) {
                inp.parentElement.classList.remove("check");
            }
        })
...
const handleQuestionNum = (event) => {
    if (event.target.tagName === "INPUT") {
        const input = event.target;
        
        userSelectQuestion.querySelectorAll("input").forEach((inp) => {
            if (input !== inp) {
                inp.parentElement.classList.remove("check");
            }
        })
...
quizConfig.addEventListener("submit", submitConfiguration);
nextBtn.addEventListener("click", updateNextQuiz);
quizOptions.addEventListener("click", checkAnswer);
quizContainer.addEventListener("submit", onSubmit);
tryAgainBtn.addEventListener("click", replayQuiz);
categoryOptions.addEventListener("click", handleConfig);
userSelectQuestion.addEventListener("click", handleQuestionNum);
